...
const treeEl = mustElement<HTMLElement>("#tree");
const searchInput = mustElement<HTMLInputElement>("#search");
const statusEl = mustElement<HTMLElement>("#status");
const contentEl = mustElement<HTMLElement>("#content");
...
const debounce = <Args extends unknown[]>(fn: (...args: Args) => void, delay = 150) => {
...
function loadCollapsedDirectories(): Set<string> {
  try {
    const paths: unknown = JSON.parse(localStorage.getItem(collapsedDirectoryStorageKey) || "[]");
    return new Set(
...
function treeToHtml(nodes: TreeNode[], isRoot = false): string {
  const items = nodes
    .map((node) => {
      if (node.type === "dir") {
        const isCollapsed = isDirectoryCollapsed(node.path, currentQuery, collapsedDirectories);
        return `<li class="dir">
...
async function fetchTree(options: { openInitial?: boolean } = {}) {
...
function closeDocumentDialogs() {
  document
    .querySelectorAll<HTMLDialogElement>("dialog.markdown-mermaid-dialog")
    .forEach((dialog) => {
...
async function openFile(path: string, options: OpenFileOptions = {}) {
  ...
  try {
    ...
    contentEl.innerHTML = `<div class="empty-state"><div class="empty-title">Could not open file</div><div class="empty-sub">${escapeHtml(message)}</div></div>`;
  }
}

function prepareDocument(path: string, generation: number) {
...
function updateUrl(path: string, replace = false, hash = "") {
...
function rewriteLocalUrls(path: string) {
  contentEl.querySelectorAll<HTMLAnchorElement>("a[href]").forEach((link) => {
...
function ensureHeadingIds() {
  ...
  const used = new Set(
    Array.from(contentEl.querySelectorAll<HTMLElement>("[id]"), (element) => element.id).filter(
      Boolean,
    ),
  );
...
function languageFor(code: HTMLElement): string | undefined {
  ...
  return requested ? aliases[requested] || requested : undefined;
}

function highlightCode() {
  contentEl.querySelectorAll<HTMLElement>("pre > code").forEach((code) => {
...
function enhanceTables() {
  ...
  const prepareColumns = (table: HTMLTableElement): HTMLTableColElement => {
    ...
    if (!table.dataset.columnsSized) {
      ...
      headers.forEach((header, index) => {
        const column = colgroup.children.item(index) as HTMLTableColElement | null;
        if (column) column.style.width = `${header.getBoundingClientRect().width}px`;
      });
  ...
  const resizeColumn = (table: HTMLTableElement, columnIndex: number, delta: number) => {
    const column = prepareColumns(table).children.item(columnIndex) as HTMLTableColElement | null;
    if (!column) return;
  ...
  contentEl.querySelectorAll<HTMLTableElement>("table").forEach((table) => {
    ...
    headers.forEach((header) => {
      ...
      const sort = () => {
        ...
        table.querySelectorAll<HTMLTableCellElement>("thead th").forEach((other) => {
      ...
      header.addEventListener("click", (event) => {
        ...
        if (!target?.closest("a, button, input, select, textarea, .markdown-column-resizer"))
          sort();
      });
...
const mermaidIcons = {
  ...
  close: [
    "M3.72 3.72a.75.75 0 0 1 1.06 0L8 6.94l3.22-3.22a.75.75 0 0 1 1.06 1.06L9.06 8l3.22 3.22a.75.75 0 0 1-1.06 1.06L8 9.06l-3.22 3.22a.75.75 0 0 1-1.06-1.06L6.94 8 3.72 4.78a.75.75 0 0 1 0-1.06Z",
  ],
  code: [
    "M5.72 3.22a.75.75 0 0 1 1.06 1.06L3.06 8l3.72 3.72a.75.75 0 1 1-1.06 1.06L1.47 8.53a.75.75 0 0 1 0-1.06l4.25-4.25Zm4.56 0 4.25 4.25a.75.75 0 0 1 0 1.06l-4.25 4.25a.75.75 0 1 1-1.06-1.06L12.94 8 9.22 4.28a.75.75 0 0 1 1.06-1.06Z",
  ],
  copy: [
  ...
  expand: [
    "M3.72 3.72a.75.75 0 0 1 1.06 1.06L2.56 7h10.88l-2.22-2.22a.75.75 0 0 1 1.06-1.06l3.5 3.5a.75.75 0 0 1 0 1.06l-3.5 3.5a.75.75 0 0 1-1.06-1.06l2.22-2.22H2.56l2.22 2.22a.75.75 0 0 1-1.06 1.06l-3.5-3.5a.75.75 0 0 1 0-1.06l3.5-3.5Z",
  ],
  reset: [
  ...
  zoomIn: [
    ...
    "M7.5 0a7.5 7.5 0 0 1 5.807 12.247l2.473 2.473a.75.75 0 0 1-1.06 1.06l-2.473-2.473A7.5 7.5 0 1 1 7.5 0Zm-6 7.5a6 6 0 1 0 12 0 6 6 0 0 0-12 0Z",
  ],
  zoomOut: [
    "M4.5 6.75h6a.75.75 0 0 1 0 1.5h-6a.75.75 0 0 1 0-1.5Z",
    "M0 7.5a7.5 7.5 0 1 1 13.307 4.747l2.473 2.473a.75.75 0 0 1-1.06 1.06l-2.473-2.473A7.5 7.5 0 0 1 0 7.5Zm7.5-6a6 6 0 1 0 0 12 6 6 0 0 0 0-12Z",
  ],
};
...
async function renderMermaidDiagrams(generation: number) {
  ...
  for (const code of sources) {
    ...
    try {
      ...
      const navigationControls: Array<[string, string, readonly string[], () => void]> = [
        [
          "Zoom in",
          "zoom-in",
          mermaidIcons.zoomIn,
          () => {
            view.scale = Math.min(5, +(view.scale + 0.1).toFixed(1));
          },
        ],
        [
          "Zoom out",
          "zoom-out",
          mermaidIcons.zoomOut,
          () => {
      ...
      const expand = makeMermaidControl(
        "Open expanded view",
        "markdown-mermaid-expand",
        mermaidIcons.expand,
      );
      expand.setAttribute("aria-haspopup", "dialog");
      const toggleSource = makeMermaidControl(
        "Show Mermaid code",
        "markdown-mermaid-toggle-source",
        mermaidIcons.code,
      );
      toggleSource.setAttribute("aria-pressed", "false");
      const copy = makeMermaidControl(
        "Copy Mermaid code",
        "markdown-mermaid-copy",
        mermaidIcons.copy,
      );
      ...
      toggleSource.addEventListener("click", () => {
        ...
        toggleSource.setAttribute(
          "aria-label",
          showSource ? "Show Mermaid diagram" : "Show Mermaid code",
        );
      ...
      copy.addEventListener("click", async () => {
        ...
        copy.setAttribute(
          "aria-label",
          copied ? "Copied Mermaid code" : "Could not copy Mermaid code",
        );
      ...
      const close = makeMermaidControl(
        "Close expanded view",
        "markdown-mermaid-dialog-close",
        mermaidIcons.close,
      );
      ...
      viewport.addEventListener(
        ...
        (event) => {
          ...
          view.scale = Math.min(
            5,
            Math.max(0.2, +(view.scale + (event.deltaY < 0 ? 0.1 : -0.1)).toFixed(1)),
          );
          updateView();
        },
        { passive: false },
      ...
      if (typeof bindFunctions === "function") bindFunctions(diagram);
    } catch (error) {
      source.classList.add("markdown-mermaid-error");
      const message = error instanceof Error ? error.message : String(error);
      source.title = `Could not render Mermaid diagram: ${message}`;
...
function buildToc() {
  const tocHandle = mustElement<HTMLElement>("#toc-resize-handle");
...
function flashHeading(heading: HTMLElement) {
  ...
  requestAnimationFrame(() =>
    requestAnimationFrame(() => heading.classList.add("markdown-heading-fading")),
  );
  window.setTimeout(
    () => heading.classList.remove("markdown-heading-flash", "markdown-heading-fading"),
    1350,
  );
}
...
function jumpToHash(hash: string, smooth = true) {
  let id: string;
  try {
  ...
  const target = Array.from(contentEl.querySelectorAll<HTMLElement>("[id]")).find(
...
function updateTocState() {
  ...
  tocHeadings.forEach((heading, index) => {
    ...
    links[index]?.classList.toggle(
      "visible",
      box.bottom > viewerBox.top && box.top < viewerBox.bottom,
    );
...
treeEl.addEventListener("click", (event) => {
  const target = event.target instanceof Element ? event.target : null;
  const directory = target?.closest<HTMLButtonElement>("button[data-dir]");
  if (directory) {
  ...
  const button = target?.closest<HTMLButtonElement>("button[data-file]");
...
contentEl.addEventListener("click", (event) => {
  const target = event.target instanceof Element ? event.target : null;
  const link = target?.closest<HTMLAnchorElement>("a[data-document-path], a[data-document-hash]");
  if (
    !link ||
  ...
  if (link.dataset.documentHash) jumpToHash(link.dataset.documentHash);
});

tocListEl.addEventListener("click", (event) => {
  const target = event.target instanceof Element ? event.target : null;
  const link = target?.closest<HTMLAnchorElement>("a[data-index]");
  if (!link) return;
  event.preventDefault();
  ...
  window.history.replaceState(
    { path: currentPath },
    "",
    `/${encodePath(currentPath)}#${encodeURIComponent(heading.id)}`,
  );
...
window.addEventListener("popstate", (event) => {
  ...
  if (path && path !== currentPath)
    void openFile(path, { updateUrl: false, hash: window.location.hash });
...
type ResizerOptions = {
  axis: "x" | "y";
  cursor: string;
  onDelta: (delta: number, event: PointerEvent | KeyboardEvent) => void;
};

function installResizer(handle: HTMLElement, options: ResizerOptions) {
...
installResizer(sidebarResizeHandle, {
  ...
  onDelta(delta) {
    const width = Math.min(
      Math.max(sidebarEl.getBoundingClientRect().width + delta, 180),
...
installResizer(tocResizeHandle, {
  ...
  onDelta(delta) {
    ...
    tocEl.style.flex = "1";
    tocResizeHandle.setAttribute("aria-valuenow", String(Math.round(height)));
  },
});

fetchTree({ openInitial: true }).catch((error) => {
  statusEl.textContent = error instanceof Error ? error.message : "Failed to load Markdown tree.";
});
